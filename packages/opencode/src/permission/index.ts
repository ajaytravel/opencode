...
export function fromConfig(permission: ConfigPermission.Info) {
  ...
  for (const [key, value] of Object.entries(permission)) {
    const normalized = ShellToolID.normalize(key)
    if (typeof value === "string") {
      ruleset.push({ permission: normalized, action: value, pattern: "*" })
      continue
    }
    ruleset.push(
      ...Object.entries(value).map(([pattern, action]) => ({ permission: normalized, pattern: expand(pattern), action })),
    )
  }
  return ruleset
}
